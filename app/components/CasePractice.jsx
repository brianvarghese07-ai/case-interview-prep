...
import {
  ...
  BookOpen, CheckCircle2, Clock, Radio,
} from 'lucide-react'
...
function TextBlock({ text, className = '' }) {
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      {text.split('\n\n').map((para, i) => (
        <p key={i} className="text-[color:var(--ink-soft)] leading-relaxed text-sm sm:text-[0.95rem] whitespace-pre-wrap">
          {para}
        </p>
      ))}
    </div>
          ...
          <p
            ...
            className={`leading-relaxed text-sm sm:text-[0.95rem] whitespace-pre-wrap ${
              line.speaker === 'candidate'
                ...
                : 'font-normal text-[color:var(--ink-soft)]'
            }`}
          >
            {line.text}
          </p>
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 relative">
      ...
      <div className="relative z-10 flex items-center gap-2 py-3 sm:py-4 text-sm">
        ...
        <span className="text-[color:var(--ink-soft)] font-medium truncate hidden sm:inline">{c.title}</span>
      </div>
        ...
        <div className="space-y-6 animate-rise">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4 pt-2">
            {prev ? (
              <Link
                href={`/cases/${prev.id}`}
                className="flex items-center gap-2 text-sm font-medium text-[color:var(--ink-soft)] hover:text-brand-600 card px-4 py-3 sm:py-2.5 hover:shadow-md transition-all group"
              >
                <ArrowLeft className="w-4 h-4 group-hover:-translate-x-0.5 transition-transform" />
                <div className="text-left">
                  <p className="text-xs text-[color:var(--muted)]">Previous</p>
            ...
            {next ? (
              <Link
                href={`/cases/${next.id}`}
                className="flex items-center gap-2 text-sm font-medium text-[color:var(--ink-soft)] hover:text-brand-600 card px-4 py-3 sm:py-2.5 hover:shadow-md transition-all group sm:ml-auto"
              >
        ...
        <div className="space-y-4 lg:sticky lg:top-24 animate-rise [animation-delay:120ms] order-last lg:order-none">
          ...
          <div className="card p-4 sm:p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-sm text-[color:var(--ink)]">Case Details</h3>
            </div>
            <dl className="grid grid-cols-2 gap-x-4 gap-y-3 sm:space-y-3 sm:block">
              {[
                ...
                <div key={label} className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1 sm:gap-3">
                  <dt className="text-[11px] sm:text-xs text-[color:var(--muted)] flex-shrink-0">{label}</dt>
                  <dd className="text-xs font-medium text-[color:var(--ink-soft)] sm:text-right break-words">{value}</dd>
                </div>
              ))}
            </dl>
          </div>
